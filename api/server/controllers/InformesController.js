...
class InformesController {
  ...
  static consolidado(req, res) {   
    Promise.all([      
      ...
      ProveedorService.getTotal(),
      ClienteService.getTotal(),
      CompraService.getTotal(),
      VentaService.getTotal(),
      NotaCobranzaService.getTotal()
    ])
      .then(([articulosT, proveedorT,clienteT,compraT,ventaT,notaT]) => {
        res.status(200).send({ result:{articulosT, proveedorT,clienteT,compraT,ventaT,notaT}});
      })
      .catch((reason) => {
         console.log(reason)
        res.status(400).send({ message: reason });
